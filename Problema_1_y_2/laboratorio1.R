...
a1c=rep(rep(c(-1,3, -5,7, -9),2),each=10)
...
dim(a2a[1:3,])
dim(a2a[-4,])
dim(a2a[-4,]) == dim(a2a[1:3,])
#PREGUNTA 2.c
a2c=a2a
a2c[,2]=sort(a2a[,2]) #se reemplazo la segunda columna con la misma columna pero con sus elementos ordenados
#PREGUNTA 2.d
is.matrix(a2a[-1,-2]) == T #comprobaremos si el resultado que devuelve la elimniacion es una matrix.
#el resultado devuelto es falso
matrix(a2a[-1,-2],3,1) #asi obtendremos una matrix de 3x1
#PREGUNTA 2.e
a2e=a2a[3:4,] #esta es una forma en la cual podemos crear la matriz 2x2
matrix(a2a[3:4,],2,2) #esta es otra forma
#PREGUNTA 2.f
a2c[c(4,1),c(2,1)]=-0.5
a2c
#PREGUNTA 2.g
A=diag(c(2,3,5,-1))
solve(A)%*%A-diag(4)== diag(0,4) #aqui podemos verificar que es igual a la matriz NULA de dim 4x4
